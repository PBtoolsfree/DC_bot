...
class AutoModConfigCog(commands.GroupCog, name="automod"):
    ...
    async def status(self, interaction: discord.Interaction) -> None:
        ...
        if not settings_db or not settings_db.enabled:
            embed = EmbedBuilder.error(
                description="AutoMod is currently **Disabled**.", title="AutoMod Status"
            )
        ...
        embed = EmbedBuilder.info(
            description="AutoMod is currently **Enabled**.", title="AutoMod Status"
        )
    ...
    async def toggle(
        self, interaction: discord.Interaction, state: discord.app_commands.Choice[str]
    ) -> None:
        ...
        embed = EmbedBuilder.success(
            description=f"AutoMod has been successfully **{status_msg}**.", title="AutoMod Toggled"
        )
        await interaction.followup.send(embed=embed, ephemeral=True)
