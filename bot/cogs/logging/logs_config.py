...
class LoggingConfigCog(commands.GroupCog, group_name="logs"):
    ...
    async def search_logs(
        ...
        if not logs:
            await interaction.followup.send(
                embed=EmbedBuilder.error("No logs found matching your criteria."), ephemeral=True
            )
            return
        ...
        for log in logs:
            time_str = (
                f"<t:{int(log.created_at.timestamp())}:R>" if log.created_at else "Unknown Time"
            )
    ...
    @app_commands.describe(
        format="The file format for the export", days="How many days of logs to export"
    )
    ...
    async def export_logs(
        self, interaction: discord.Interaction, format: str = "csv", days: int = 30
    ) -> None:
        if not interaction.guild:
            return
        ...
        if not logs:
            await interaction.followup.send(
                embed=EmbedBuilder.error("No logs found to export."), ephemeral=True
            )
            return
