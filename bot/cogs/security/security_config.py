...
class SecurityConfigCog(commands.GroupCog, group_name="security"):
    ...
    @app_commands.command(name="status", description="View the current security status and health score")
    ...
    async def status(self, interaction: discord.Interaction) -> None:
        ...
        async with self.bot.db.session() as session:
            settings_db = await GuildRepository.get_module_settings(session, interaction.guild.id, "security")
        ...
        embed.add_field(name="Health Score", value=f"**{health_score}/100** ({health_desc})", inline=False)
    ...
    @app_commands.command(name="snapshot", description="Create a manual backup snapshot of the server")
    @require_permission(PermissionLevel.ADMIN)
