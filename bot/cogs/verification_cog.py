    ...
    async def verify_setup(self, interaction: discord.Interaction) -> None:
        ...
        async with db.session() as session:
            ...
            if not settings or not settings.enabled:
                await interaction.followup.send(
                    "Please enable Verification in the Dashboard first."
                )
                return
        ...
        embed = discord.Embed(
            title=locales.get_string(
                settings.language,
                "verification",
                "BUTTON_VERIFICATION_TITLE",
                fallback="Server Verification",
            ),
            description=locales.get_string(
                settings.language,
                "verification",
                "BUTTON_VERIFICATION_DESC",
                fallback="Click the button below to verify.",
            ),
