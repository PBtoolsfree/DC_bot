...
class ReactionRolePanel(Base):
    ...
    group_id: Mapped[int] = mapped_column(
        ForeignKey("reaction_role_groups.id", ondelete="CASCADE"), nullable=False
    )
...
class ReactionRoleGroup(Base):
    ...
    required_roles: Mapped[list[int]] = mapped_column(
        JSON, default=list
    )  # User must have these to use
    blacklisted_roles: Mapped[list[int]] = mapped_column(
        JSON, default=list
