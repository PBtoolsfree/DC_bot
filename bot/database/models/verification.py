...
class VerificationSettings(Base):
    ...
    risk_threshold_high: Mapped[int] = mapped_column(Integer, default=70, nullable=False) # e.g. score > 70 requires Image/Manual
...
class VerificationSession(Base):
    ...
    session_id: Mapped[str] = mapped_column(String(64), index=True, unique=True, nullable=False) # UUID or HMAC
...
class VerificationHistory(Base):
    ...
    timestamp: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True, server_default=func.now(), nullable=False)
