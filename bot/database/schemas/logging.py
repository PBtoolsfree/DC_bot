...
class LogChannelConfig(BaseModel):
    ...
    channel_id: int | None = Field(
        default=None, description="The Discord Channel ID to send logs to."
    )
    events: list[LogEvent] = Field(
        default_factory=list, description="List of events routed to this channel."
    )
...
class LoggingSettings(BaseModel):
    ...
    retention_days: int = Field(
        default=30, ge=1, le=365, description="Number of days to keep logs in DB."
    )
