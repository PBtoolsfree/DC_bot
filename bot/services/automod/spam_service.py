...
class SpamService:
    ...
    async def check_message(self, message: discord.Message, settings: AutoModSettings) -> str | None:
        ...
        if settings.spam_mentions.enabled and not self._check_ignored(message, settings.spam_mentions):
        ...
        if settings.spam_mass_mentions.enabled and not self._check_ignored(message, settings.spam_mass_mentions):
            mention_count = len(message.raw_mentions) + len(message.raw_role_mentions)
            if mention_count > 0:
        ...
        if settings.spam_attachments.enabled and not self._check_ignored(message, settings.spam_attachments):
        ...
        if settings.spam_duplicates.enabled and content and not self._check_ignored(message, settings.spam_duplicates):
        ...
        if settings.spam_messages.enabled and not self._check_ignored(message, settings.spam_messages):
