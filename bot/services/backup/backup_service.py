...
class BackupService:
    ...
    @staticmethod
    async def create_backup(session: AsyncSession, guild: discord.Guild, creator_id: int, name: str, description: str | None = None) -> ServerBackup:
        """Serializes roles, channels, and categories."""
        
        payload = {
            "roles": [
                {"id": r.id, "name": r.name, "color": r.color.value, "permissions": r.permissions.value, "position": r.position}
