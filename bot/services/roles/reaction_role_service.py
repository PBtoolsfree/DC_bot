...
class ReactionRoleService:
    ...
    async def toggle_role(
        session: AsyncSession, member: discord.Member, group_id: int, role_id: int
    ) -> tuple[bool, str]:
        ...
        if role in member.roles:
            ...
            if group.min_roles > 0:
                ...
                stmt_items = select(ReactionRoleItem.role_id).where(
                    ReactionRoleItem.group_id == group_id
                )
                ...
                if user_group_roles_count <= group.min_roles:
                    return (
                        False,
                        f"You must have at least {group.min_roles} role(s) from this group.",
                    )
