...
class RollbackService:
    ...
    async def restore_channel(
        guild: discord.Guild, audit_entry: discord.AuditLogEntry
        ...
        try:
            ...
            new_channel = await guild.create_channel(
                ...
                type=channel_type,
                category=(
                    guild.get_channel(getattr(before, "category_id", None))
                    if getattr(before, "category_id", None)
                    else None
                ),
    ...
    async def strip_dangerous_roles(
        member: discord.Member, reason: str = "Anti-Nuke: Stripping Dangerous Roles"
    ) -> None:
