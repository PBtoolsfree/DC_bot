...
class SecurityService:
    ...
    def __init__(
        self, 
        raid_service: RaidDetectionService,
    ...
    async def handle_action(
        ...
        self._broadcast_dashboard_event(guild.id, "security_trigger", {"action": action_type, "executor": executor.id})
    ...
    async def _apply_punishment(self, member: discord.Member, action: str, reason: str) -> bool:
        ...
        try:
            ...
            if action == "ban":
                ...
                await member.timeout(datetime.now(timezone.utc) + timedelta(hours=1), reason=full_reason)
            return True
    ...
    async def _queue_rollback(self, guild: discord.Guild, action_type: str, audit_entry: discord.AuditLogEntry) -> str:
