...
class WelcomeService:
    ...
    async def handle_member_join(self, session: AsyncSession, member: discord.Member) -> None:
        ...
        content = self._format_message(settings.welcome_message or "Welcome {user} to {guild}!", member)
        
        # Generate Image if URL provided
        file = None
        if settings.welcome_image_url:
            try:
                img_bytes = await self.image_provider.generate_welcome_card(member, settings.welcome_image_url)
                import io
