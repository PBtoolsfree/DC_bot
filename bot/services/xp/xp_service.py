    ...
    async def handle_message(self, session: AsyncSession, message: discord.Message) -> None:
        ...
        if xp_gained > 0:
            ...
            await StreamingService.broadcast(
                ...
                payload={"user_id": str(message.author.id), "amount": xp_gained, "source": "message"}
            )

    async def _check_level_up(self, session: AsyncSession, guild: discord.Guild, member: discord.Member) -> None:
        ...
        if new_level > user_xp.level:
            ...
            # Apply Rewards
            stmt_rew = select(XPReward).where(XPReward.guild_id == guild.id, XPReward.level == new_level)
