...
class TicketPanelView(View):
    ...
    def __init__(self, panel_id: int, categories: list):
        ...
        self.select = Select(placeholder="Select a category...", options=options, custom_id=f"panel_{panel_id}")
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        async with db.session() as session:
            ...
            if ticket.channel_id:
                await interaction.followup.send(f"Ticket opened: <#{ticket.channel_id}>", ephemeral=True)
                
                # Send control view to the new channel
                channel = interaction.guild.get_channel(ticket.channel_id) # type: ignore
                if channel:
                    embed = discord.Embed(title=f"Ticket: {category.name}", description="Support will be with you shortly.")
                    await channel.send(embed=embed, view=TicketControlView(ticket.id)) # type: ignore
            else:
                await interaction.followup.send("Ticket opened anonymously. We will DM you.", ephemeral=True)
