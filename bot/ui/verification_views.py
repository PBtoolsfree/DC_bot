...
class VerificationModal(Modal):
    ...
    def __init__(self, token: str, language: str):
        title = locales.get_string(language, "verification", "CAPTCHA_TITLE", fallback="Solve Captcha")
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        async with db.session() as session:
            ...
            if success:
                ...
                msg = locales.get_string(self.language, "verification", "VERIFICATION_FAILED", attempts=settings.max_attempts)
                await interaction.followup.send(msg, ephemeral=True)
...
class VerificationView(View):
    ...
    def __init__(self, language: str):
        ...
        btn = Button(
            label=locales.get_string(language, "verification", "VERIFY_BUTTON_LABEL", fallback="Verify"),
    ...
    async def start_verification(self, interaction: discord.Interaction) -> None:
        ...
        async with db.session() as session:
            ...
            if v_session.verification_type == "button":
                ...
                success = await service.process_answer(session, interaction.user, v_session.session_id, "", settings)
                msg = locales.get_string(settings.language, "verification", "VERIFICATION_SUCCESS" if success else "VERIFICATION_FAILED")
                await interaction.followup.send(msg, ephemeral=True)
                return
            ...
            class AnswerView(View):
                ...
                async def ans(self, interaction: discord.Interaction, button: Button):
                    await interaction.response.send_modal(VerificationModal(v_session.session_id, settings.language))
