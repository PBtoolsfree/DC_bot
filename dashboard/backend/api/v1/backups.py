...
async def list_backups(
    ...
    has_perm = await RBACService.has_permission(session, guild_id, current_user["id"], "manage_backups")
    if not has_perm:
        raise HTTPException(status_code=403, detail="Forbidden")

    stmt = select(ServerBackup).where(ServerBackup.guild_id == guild_id).order_by(ServerBackup.created_at.desc())
