...
from bot.database.models.tickets import Ticket, TicketPanel
...
async def get_ticket_analytics(
    ...
    stmt_open = select(func.count(Ticket.id)).where(
        ...
        Ticket.status.in_(["open", "claimed", "in_progress", "waiting_user", "waiting_staff"]),
    )
    ...
    stmt_closed = select(func.count(Ticket.id)).where(
        Ticket.guild_id == guild_id, Ticket.status == "closed"
    )
...
async def get_ticket_panels(
    ...
    return [
        {"id": p.id, "title": p.title, "channel_id": str(p.channel_id), "categories": p.categories}
        for p in panels
    ]
