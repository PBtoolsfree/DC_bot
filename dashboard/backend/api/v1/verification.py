...
from dashboard.shared.schemas.verification import VerificationSettingsUpdate, VerificationSettingsResponse
...
async def get_verification_settings(
    ...
    has_perm = await RBACService.has_permission(session, guild_id, current_user["id"], "manage_verification")
    if not has_perm:
    ...
    return VerificationSettingsResponse(
        ...
        quarantine_role_id=str(settings.quarantine_role_id) if settings.quarantine_role_id else None,
        ...
        verification_channel_id=str(settings.verification_channel_id) if settings.verification_channel_id else None,
...
async def update_verification_settings(
    ...
    for key in ["quarantine_role_id", "verified_role_id", "temporary_role_id", "verification_channel_id", "log_channel_id"]:
