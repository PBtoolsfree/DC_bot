...
async def get_welcome_settings(
    ...
    has_perm = await RBACService.has_permission(session, guild_id, current_user["id"], "manage_welcome")
    if not has_perm:
    ...
    return WelcomeSettingsSchema(
        ...
        welcome_channel_id=str(settings.welcome_channel_id) if settings.welcome_channel_id else None,
        welcome_message=settings.welcome_message,
        welcome_image_url=settings.welcome_image_url,
        goodbye_enabled=settings.goodbye_enabled,
        goodbye_channel_id=str(settings.goodbye_channel_id) if settings.goodbye_channel_id else None,
